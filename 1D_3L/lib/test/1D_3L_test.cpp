#include "1D_3L/model_inference.h"
#include <iostream>
#include <fstream>
#include <string>
#include <sstream>
...
vector<double> parse_inputs(const string &line) {
    ...
    for (int i = 0; i < inference_1d_3l::INPUTS_COUNT; i++) {
        getline(input, token, ',');
...
void write_header(ofstream &ofs) {
    for (int i = 0; i < inference_1d_3l::OUTPUTS_COUNT; i++) {
        const string token = inference_1d_3l::OUTPUTS[i];
        ofs << token;
        
        if (i < inference_1d_3l::OUTPUTS_COUNT - 1) {
            ofs << ",";
        }
    }
    ofs << endl;
...
int main(int argc, const char* argv[]) {
    ...
    while (true) {
        ...
        vector<double> predictions = inference_1d_3l::calc_predictions(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4]);
