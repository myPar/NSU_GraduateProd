...
vector<double> parse_inputs(const string &line) {
    ...
    for (int i = 0; i < inference_1d_3l_chart::INPUTS_COUNT; i++) {
        getline(input, token, ',');
...
void write_header(ofstream &ofs) {
    for (int i = 0; i < inference_1d_3l_chart::OUTPUTS_COUNT; i++) {
        const string token = inference_1d_3l_chart::OUTPUTS[i];
        ofs << token;
        
        if (i < inference_1d_3l_chart::OUTPUTS_COUNT - 1) {
            ofs << ",";
        }
    }
    ofs << endl;
