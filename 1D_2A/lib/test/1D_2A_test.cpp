#include "1D_2A/model_inference.h"
#include <iostream>
#include <fstream>
#include <string>
#include <sstream>
...
const int OUTPUTS_ATTR_COUNT = 5;
...
void write_header(ofstream &ofs) {
    for (int i = 0; i < OUTPUTS_ATTR_COUNT; i++) {
        const string token = inference_1d_2a::OUTPUTS[i];
        ofs << token;
...
int main(int argc, const char* argv[]) {
    ...
    while (true) {
        ...
        vector<double> predictions = inference_1d_2a::calc_predictions(inputs[0], inputs[1], inputs[2], inputs[3]);
