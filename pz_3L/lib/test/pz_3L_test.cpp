...
vector<double> parse_inputs(const string &line) {
    ...
    for (int i = 0; i < inference_pz_3l::INPUTS_COUNT; i++) {
        getline(input, token, ',');
...
void write_header(ofstream &ofs) {
    for (int i = 0; i < inference_pz_3l::OUTPUTS_COUNT; i++) {
        const string token = inference_pz_3l::OUTPUTS[i];
        ofs << token;
        
        if (i < inference_pz_3l::OUTPUTS_COUNT - 1) {
            ofs << ",";
        }
    }
    ofs << endl;
